        ...
        for line in lines:
            ...
            # Read Struts
                ...
                if not line:  # end of the strut section
                    self.struts.append(
                        current_strut
                    )  # add the strut to the struts list of the kite
                    current_strut = Strut()
                    continue
...
def plot_data(rib_data, le_tube_data, struts_data, bridle_data):
    ...
    if (
        len(rib_data) == 0
        and le_tube_data.size == 0
        and len(struts_data) == 0
        and len(bridle_data) == 0
    ):
    ...
    for bridle in bridle_data:
        ...
        ax.plot(
            [bridle.top[0], bridle.bottom[0]],
            [bridle.top[1], bridle.bottom[1]],
            [bridle.top[2], bridle.bottom[2]],
            c="r",
            label=label,
        )
        first_bridle = False
